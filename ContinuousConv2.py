...
FreqCompRows = np.fft.fftfreq(ft.shape[0],d=2)
FreqCompCols = np.fft.fftfreq(ft.shape[0],d=2)
...
ft2 = np.abs(ft)
print (ft)
np.savetxt('fftSamples.txt',ft2, fmt='%.3f', delimiter = ',')
g = [column[0] for column in dataSamples[1:len(dataSamples)]]
h = [column[1] for column in ft2[1: len(ft2)]]
